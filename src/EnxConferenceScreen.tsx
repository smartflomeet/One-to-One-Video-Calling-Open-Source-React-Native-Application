...
export default class EnxVideoView extends PureComponent<Props, State> {
  private backHandler: { remove: () => void } | null = null; // Declare the type of backHandler
  ...
  cleanUpAndGoBack = () => {
    try {
      ...
      this.props.navigation.goBack(); // Fallback in case of errors
    }
  };
    ...
    this.roomEventHandlers = {
      ...
      roomDisconnected: event => {
        ...
        this.props.navigation.dispatch(
          CommonActions.reset({
            index: 0,
            routes: [{ name: 'EnxJoinScreen' }],
          })
        ); // if you don't want to reset stack
      },
      recordStarted: event => {
